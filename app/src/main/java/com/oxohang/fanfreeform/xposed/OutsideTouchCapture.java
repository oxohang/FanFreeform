...
final class OutsideTouchCapture {
    ...
    private void updateNow(List<Rect> windows, Rect ime,
        ...
        // Stay below Xiaomi's GestureStub (type 2027), so edge swipes retain the
        // native back animation while the same outside regions still block apps.
        if (!attach(regions, clippedWindows.size(), TYPE_NAVIGATION_BAR_PANEL)
